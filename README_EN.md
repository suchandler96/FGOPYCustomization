...
## Grammar
The sequence of actions needs to be placed in a separate file, e.g., `SampleTurnSeq.txt`, `WhitePaper90SS.txt` provided in the repo. It roughly follows python-style if-else sequence and indent rules, in-line if-else branches, and supports `#`-style comments. If passed to `install.py`, the script will automatically translate the file into python class and install it to `fgoKernel.py`.
