...
This feature is only supported when running FGO-py in command line interface (CLI) mode. The sequence of actions needs to be placed in a separate file, e.g., `SampleTurnSeq.txt`, `WhitePaper90SS.txt` provided in the repo. If passed to `install.py` with the `-f` option, the script will automatically translate the file into python class and install it to `fgoKernel.py`. The `-f` option can be followed by multiple files for customization. At runtime, extra parameters can be added to the `main` or `battle` command to choose which logic to use.
