...
|   |-- ...
```
2. 将本项目提供的补丁和定制策略安装到FGO-py项目中：`cd FGOPYCustomization/ && python3 install.py -f ../FGO-py/FGO-py/customTurn.py`。若运行`install.py`时不指定`-f`参数，则不安装定制策略，仅打补丁。
...
## 适用范围与使用方法
目前只支持在命令行界面（CLI）模式下运行FGO-py时使用该功能。这串行动代码需要放在单独的文件中，例如本仓库提供的`SampleTurnSeq.txt`和`WhitePaper90SS.txt`。如果将该文件作为`-f`的参数传给`install.py`，它会自动将其翻译成Python的类并复制到`fgoKernel.py`中。`-f`后可以加多个定制逻辑的描述文件，在运行时可以在`main`或`battle`命令后加额外参数选择用哪个逻辑打当前副本。
...
```
其中`<CustomTurnName>`是用户提供的txt文件名的前缀，例如`SampleTurnSeq`或`WhitePaper90SS`。但为了方便起见，用户也可以再不引起歧义时只提供开头几个字符，如`-f Sam`或`-f Wh`。如果发现有多个匹配项则会不予安装。
